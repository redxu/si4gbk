...
)
{
	...
	si_file_info = FindSiFileFromLink(hash);
OutputDebugStringEx("read %s", orgfilename); 
	if(si_file_info == NULL)
	{
...
OutputDebugStringEx("%d", __LINE__);   	
    	DWORD fread;
    	DWORD fsize = GetFileSize(hFile,NULL);
    	char* buffer = (char*)malloc(fsize+1);
    	memset(buffer,0,fsize+1);
    	...
    	u8flag = IsUtf8(buffer,fsize);
    	//debug
    	OutputDebugStringEx("%d[%s]", u8flag, orgfilename);	
    	...
    	{    		
    		...
    		HANDLE hGbk = OrgCreateFileW(lpwszStr,
								GENERIC_WRITE,
    							0,
    							NULL,
    							CREATE_ALWAYS,
    	...
    	free(buffer);
		SiFile_Add(hash,u8flag,fmd5,(char *)orgfilename,hookfilename);	
	}
	else 
	{		
		u8flag = si_file_info->u8flag;
		...
		if(u8flag != 0)
		{
			if(dwDesiredAccess == GENERIC_READ)
			{
				...
				if(memcmp(fmd5,si_file_info->orgmd5,16) != 0)
				{
					OutputDebugStringEx("u8[%s] Changed outside!",orgfilename);
					//convert
					DWORD gbksize = 0;
		    		DWORD gbkwriten;
		    		char* gbk = (char *)malloc(fsize+1);
...
RECOVER:
	MByteToWChar(hookfilename, lpwszStr, 1024);
OutputDebugStringEx("%d %s", __LINE__, hookfilename);
	handle = OrgCreateFileW(lpwszStr,dwDesiredAccess,dwShareMode,lpSecurityAttributes,
									dwCreationDisposition,dwFlagsAndAttributes,hTemplateFile);

	if(u8flag != 0)
	{
		SiHandle_Add(handle,u8flag,(char *)orgfilename,hookfilename);		
	}
...
)
{
	...
	if(si_handle_info != NULL)
	{
    	...
    	//写回utf8
    	HANDLE hUtf8 = OrgCreateFileA(si_handle_info->orgfile,
								GENERIC_WRITE,
    							0,
    							NULL,
    							CREATE_ALWAYS,
